...
namespace PixivStd
{
    ...
    public class PixivBaseAPI
    {
        ...
        public int RefreshInterval
        {
            ...
            set
            {
                ...
                if (value > 0)
                    ...
                    refreshTimerKai.Interval = TimeSpan.FromMinutes(value).TotalMilliseconds;
                
            }
        }
        //DispatcherTimer refreshTimer = new DispatcherTimer();
        System.Timers.Timer refreshTimerKai = new System.Timers.Timer();
            ...
            bool ExperimentalConnection = false, int RefreshInterval = 45)
        {
            ...
            refreshTimerKai.Interval = TimeSpan.FromMinutes(RefreshInterval).TotalMilliseconds;
            refreshTimerKai.Elapsed += RefreshTimer_Tick;
        ...
        public async Task<JsonObject> Auth(string Username, string Password)
        {
            ...
            var resJSON = JsonValue.Parse(await GetResponseString(res));
            AccessToken = resJSON["response"]["access_token"];
            UserID = resJSON["response"]["user"]["id"];
            RefreshToken = resJSON["response"]["refresh_token"];
            if (RefreshInterval > 0) //refreshTimer.Start();
                refreshTimerKai.Start();
            return resJSON as JsonObject;
        }
        ...
        public async Task<JsonObject> Auth(string RefreshToken)
        {
            ...
            AccessToken = resJSON["response"]["access_token"];
            UserID = resJSON["response"]["user"]["id"];
            this.RefreshToken = resJSON["response"]["refresh_token"];
            if (RefreshInterval > 0)// refreshTimer.Start();
                refreshTimerKai.Start();
            return resJSON as JsonObject;
        }
